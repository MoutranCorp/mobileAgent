...
- **Cross-platform contract:** Android/proot-Debian is the primary deployment
  runtime, but shared broker/web code must run on Windows too. Use Node path APIs
  instead of hardcoded `/tmp` or `/sdcard` in shared code; use `spawn` argument
  arrays instead of shell interpolation; document both PowerShell and Bash paths
